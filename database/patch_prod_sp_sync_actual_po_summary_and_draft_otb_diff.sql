-- Production patch: Actual PO business key + summary sync + PO matching + Draft OTB diff
-- Date: 2026-06-08
-- Scope:
--   1. Drop obsolete dbo.Actual_PO_Summary unique constraint on (PO_No, PO_Item), if present
--   2. Rebuild dbo.Actual_PO_Summary business-key lookup index
--   3. dbo.SP_Sync_Actual_PO_Summary
--   4. dbo.SP_Get_Draft_OTB_Diff
--   5. dbo.SP_Auto_Match_Actual_Draft
--   6. Post-deploy data repair for legacy Unmatched/changed Actual PO state
-- Run against the BMS database.

USE [BMS];
GO

IF OBJECT_ID(N'dbo.Actual_PO_Summary', N'U') IS NOT NULL
   AND EXISTS
   (
        SELECT 1
        ...
        WHERE kc.parent_object_id = OBJECT_ID(N'dbo.Actual_PO_Summary')
          AND kc.[type] = N'UQ'
          AND kc.[name] = N'UQ_Actual_PO_Summary'
        GROUP BY kc.parent_object_id, kc.unique_index_id
        HAVING COUNT(*) = 2
           AND SUM(CASE WHEN ic.key_ordinal = 1 AND c.[name] = N'PO_No' THEN 1 ELSE 0 END) = 1
           AND SUM(CASE WHEN ic.key_ordinal = 2 AND c.[name] = N'PO_Item' THEN 1 ELSE 0 END) = 1
   )
BEGIN
    ALTER TABLE dbo.Actual_PO_Summary DROP CONSTRAINT UQ_Actual_PO_Summary;
END;
GO
...
IF OBJECT_ID(N'dbo.Actual_PO_Summary', N'U') IS NOT NULL
   ...
   (
        SELECT 1
        FROM sys.indexes
        WHERE object_id = OBJECT_ID(N'dbo.Actual_PO_Summary')
          AND [name] = N'IX_Actual_PO_Summary_BusinessKey'
   )
BEGIN
    DROP INDEX IX_Actual_PO_Summary_BusinessKey ON dbo.Actual_PO_Summary;
END;
GO

IF OBJECT_ID(N'dbo.Actual_PO_Summary', N'U') IS NOT NULL
BEGIN
    CREATE NONCLUSTERED INDEX IX_Actual_PO_Summary_BusinessKey
        ON dbo.Actual_PO_Summary
        (
            PO_No, OTB_Year, OTB_Month, Company_Code,
            Vendor_Code, Clean_Segment, Category_Code, Brand_Code
        )
        INCLUDE (Segment_Code, Draft_PO_ID_Ref, Draft_PO_Ref, [Status], Matching_Date, Amount_THB, Amount_CCY, Actual_PO_Date);
END;
GO
...
PRINT N'Post-deploy repair: normalize Actual/Draft PO matching state.';
GO

DECLARE @PatchBy nvarchar(100) = N'Production Patch 2026-06-08';
DECLARE @RunLegacyUnmatchedCleanup bit = 1;
DECLARE @RunSummarySync bit = 0;
DECLARE @RunAutoMatch bit = 0;
DECLARE @LegacyUnmatchedActualRows int = 0;

BEGIN TRY
    BEGIN TRANSACTION;

    IF @RunLegacyUnmatchedCleanup = 1
    BEGIN
        -- Legacy cleanup only: Unmatched is not a valid business status in this flow.
        -- Open Actual PO rows must be Active; zero-amount rows must be Cancelled.
        UPDATE a
            SET a.[Status] = CASE WHEN ISNULL(a.Amount_THB, 0) = 0 THEN N'Cancelled' ELSE N'Active' END,
                a.Draft_PO_ID_Ref = NULL,
                a.Draft_PO_Ref = NULL,
                a.Matching_Date = NULL,
                a.Status_Date = GETDATE(),
                a.Status_By = @PatchBy,
                a.Changed_By = @PatchBy,
                a.Changed_date = GETDATE(),
                a.Updated_By = @PatchBy,
                a.Updated_Date = GETDATE()
        FROM dbo.Actual_PO_Summary a WITH (UPDLOCK, HOLDLOCK)
        WHERE ISNULL(a.[Status], N'') = N'Unmatched';

        SET @LegacyUnmatchedActualRows = @@ROWCOUNT;
    END;

    COMMIT TRANSACTION;

    -- Keep these disabled by default to avoid changing user work during deploy.
    -- Enable only during an agreed maintenance window if you want this script to
    -- immediately rebuild Actual summary and auto-match eligible Draft/Actual rows.
    IF @RunSummarySync = 1
    BEGIN
        EXEC dbo.SP_Sync_Actual_PO_Summary;
    END;

    IF @RunAutoMatch = 1
    BEGIN
        EXEC dbo.SP_Auto_Match_Actual_Draft @UpdateBy = @PatchBy;
    END;

    SELECT
        N'Completed' AS PatchStatus,
        @LegacyUnmatchedActualRows AS LegacyUnmatchedActualRowsNormalized,
        @RunSummarySync AS SummarySyncExecuted,
        @RunAutoMatch AS AutoMatchExecuted;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    THROW;
